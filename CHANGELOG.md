...
* Added first-run setup checklist with `devmemory setup --checklist`.
* Added recommended next-step guidance with `devmemory setup --next`.
...
* Updated CLI help output to include new setup guidance options.
* Updated package smoke tests to verify setup guidance commands.
...
* These changes are currently available on the `dev` branch and are planned for the next stable release.
